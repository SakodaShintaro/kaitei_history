...
Score Position::initScore() {
    ...
    if (eval_params.empty()) {
        //gccの方だとeval_paramsよりPosition(shared_dataに置いているやつ)が先に初期化されるようで、アクセスしにいって落ちてしまう
        //shared_dataはusinewgameのタイミングで再度初期化されるのでここでは正しく評価値が入ってなくてもいい
        return Score(0);
    }
...
Features Position::makeEvalElements() const {
	...
	for (Square sq : SquareList) {
		if (board_[sq] != EMPTY && kind(board_[sq]) != KING) {
			ee.piece_state_list.push_back(pieceState(board_[sq], sq));
		}
	}
	for (Piece p = PAWN; p <= ROOK; p++) {
		if (hand_[BLACK].num(p) > 0) {
			ee.piece_state_list.push_back(pieceState(kind(p), hand_[BLACK].num(p), BLACK));
		}
		if (hand_[WHITE].num(p) > 0) {
			ee.piece_state_list.push_back(pieceState(kind(p), hand_[WHITE].num(p), WHITE));
		}
	}
	return ee;
}
...
Score Position::calcScoreDiff() {
    ...
    //KPやPPを足し引きするのは似た動作なのでまとめる
    //ある一つの特徴が追加された、あるいは削除されたときの差分を計算
    auto calcDiffOnePieceState = [&](PieceState ps, bool isAdd) {
        int k = (isAdd ? 1 : -1);
        //KP
        kp_score_ += k * static_cast<int>(eval_params.kp(ee_.black_king_sq, ps));
        kp_score_ -= k * static_cast<int>(eval_params.kp(ee_.white_king_sq_reversed, invPieceState(ps)));

        //PP
        //addはpiece_state_listの後ろに入るからこれでいける
        for (auto i = 0; i < ee_.piece_state_list.size() - added_list_.size(); ++i) {
            pp_score_ += k * static_cast<int>(eval_params.pp(ps, ee_.piece_state_list[i]));
        }
    };
    ...
    if (kind(move.subject()) == KING) {
        ...
        if (move.capture() != EMPTY) {
            ...
            //KPは変な値になるけど、後で再計算する
            //動かす前の分を引く
            for (PieceState removed : removed_list_) {
                calcDiffOnePieceState(removed, false);
            }
            for (auto i = 0; i < removed_list_.size(); ++i) {
                for (auto j = i; j < removed_list_.size(); ++j) {
                    pp_score_ -= eval_params.pp(removed_list_[i], removed_list_[j]);
                }
            }

            //動かした後の分を加える
            for (PieceState added : added_list_) {
                calcDiffOnePieceState(added, true);
            }
            for (auto i = 0; i < added_list_.size(); ++i) {
                for (auto j = i; j < added_list_.size(); ++j) {
                    pp_score_ += eval_params.pp(added_list_[i], added_list_[j]);
                }
            }
        }
        ...
        //本来、先手後手のどっちかの玉だけが動くわけだけど、場合分けすると地獄っぽいので効率は悪いかもしれないけど再計算してしまおう
        for (PieceState ps : ee_.piece_state_list) {
            kp_score_ += eval_params.kp(ee_.black_king_sq, ps);
            kp_score_ -= eval_params.kp(ee_.white_king_sq_reversed, invPieceState(ps));
        }
    ...
    return piece_score_ + kp_score_ + pp_score_;
}
