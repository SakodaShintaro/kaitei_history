...
//学習情報を書き込むファイル
static std::ofstream loss_history("loss_history.txt");
...
void BonanzaMethod() {
    ...
    //学習パラメータの設定を書きだしておく
    loss_history << "最初のabs : " << eval_params.abs() << ", 学習する棋譜の数 : " << games.size() << ", ミニバッチサイズ : " << MINI_BATCH_SIZE << ", L1正則化項の係数 : " << L1_GRAD_COEFFICIENT
        << ", 学習探索の深さ : " << learn_depth << ", 学習率 : " << learn_rate << ", 損失関数 : " <<  loss_function_mode << ", スレッド数 : " << thread_num << std::endl;
    ...
    while (true) {
        ...
        for (int j = 0; j < num_moves; j++) {
            ...
            for (Move& move : move_list) {
                ...
                if (pv.size() == 0) {
                    //ときどき0になってることは確認されたけど、とくにどうしようもなくダメということもないので気にしなくていいかぁ
                }
            ...
            if (fail_move_num > 0) {
                for (unsigned int m = 1; m < move_and_pv.size(); m++) {
                    ...
                    MUTEX.lock();
                    //高く評価しすぎた手について勾配を増やす
                    grad.updateParams(leaf_element, delta);
                    //教師手について勾配を減らす
                    grad.updateParams(teacher_leaf_element, -delta);
                    MUTEX.unlock();
                }
            }
        ...
        if (++learned_games_num == MINI_BATCH_SIZE) {
            ...
            //パラメータ更新
            eval_params.updateParams(grad, learn_rate);
            ...
            std::cout << "SumLoss = " << Loss << ", L1_PENALTY = " << L1_GRAD_COEFFICIENT * eval_params.abs() << ", loss_average = " << Loss / learned_position_num
                << ", Percentage of correct move = " << (double)succeeded_position_num / learned_position_num * 100 << ", grad.max_value() = " << grad.max_value()
...
void prepareForLearn() {
    //学習をするための準備
    //すべて0の評価関数を書き込んだファイルを準備するだけ
    eval_params.clear();
    eval_params.writeFile();
}
