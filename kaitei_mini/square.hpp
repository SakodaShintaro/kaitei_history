...
inline static ControlDir DirToControlDir(const Dir d) {
    return dirToControlDir[d + DirOffset];
}

inline static Dir ConDirToOppositeDir(const ControlDir cd) {
    switch (cd) {
    case Con_UU:
    case Con_UUU:
        return D;
...
inline static ControlDir DirToOppositeJumpControl(const Dir d) {
...
static ControlDir JumpControlToShadowControl(const ControlDir cd) {
    switch (cd) {
    case Con_UU:
        return Con_UUU;
    case Con_RURU:
